...
- [ ] T002 [US1] 将接受协作后的调度动作改为强制 `task_call("WantedQuests")`
  - scope: `tasks/base_task.py`
  - maps_to: US1, FR-001, FR-002, Decision 1
  - verify: 接受协作后日志出现 `Task call: wanted_quests` 或等价调度日志。

- [ ] T003 [US1] 避免普通组队邀请误触发 `WantedQuests`
  - scope: `tasks/base_task.py`
...
**目标**: 确保 `oas_main` 的调度规则不会吞掉大号 `WantedQuests`。

- [ ] T004 [US1] 增加或扩展配置检查脚本
  - scope: `scripts/`, `AGENTS.md`
  - maps_to: US1-5, FR-005, Decision 2
  - verify: 脚本能检查 `wanted_quests.scheduler` 和调度规则，并输出是否可达。
...
- [ ] T006 [US1] Windows 实机验证
  - scope: `oas_main`, `oas_findjade`
  - maps_to: US1-1, US1-2, Quality 一致性
  - verify: 小号发送勾协，大号接受后进入 `WANTEDQUESTS`，并执行或明确记录执行失败原因。
...
| US1-2 已禁用悬赏时仍触发 | T002, T004, T006 |
...
| 架构决策 / 质量属性 | 对应任务 | 验证任务 |
|----------------------|----------|----------|
| Decision 1: 使用 `task_call` | T002 | T006 |
